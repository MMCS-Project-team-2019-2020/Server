...
else
{
	...
	if ($res > 0) {
		...
		exit(json_encode(array('response' => array('erorr' => 'id_owner does not exist', 'status' => 0))));
	if ($res > 0) {
		...
		exit(json_encode(array('response' => array('erorr' => 'id_recipient does not exist', 'status' => 0))));
	if ($res > 0) {
		$res = mysqli_query($link,"SELECT card_id FROM registry WHERE recipient_id = '$id_recipient' AND card_id = '$id_card'");
		$num = mysqli_num_rows($res);
			if($num != 0)
				exit(json_encode(array('response' => array('erorr' => 'id_card already exists with this user.', 'status' => 0))));
		...
		exit(json_encode(array('response' => array('erorr' => 'id_card does not exist for this user', 'status' => 0))));
}
