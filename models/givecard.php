...
else
{
	...
	if ($res > 0) {
		$res = mysqli_query($link,"SELECT name FROM card WHERE id = '$id_card' AND owner_id = '$id_owner'");
		$res = mysqli_num_rows($res);
	}
	else
	...
	if ($res > 0) {
		...
		$num = mysqli_num_rows($res);
			if($num != 0)
				exit(json_encode(array('response' => array('id_card' => 'already exists with this user.', 'status' => 0))));
		...
		exit(json_encode(array('response' => array('id_card' => 'does not exist for this user', 'status' => 0))));
}
